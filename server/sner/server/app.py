...
from sner.server.extensions import api, cors, csrf, db, login_manager, migrate, oauth, sess, webauthn
from sner.server.frontend.views import blueprint as frontend_blueprint
from sner.server.frontend.views import pagenotfound_route
...
from sner.server.utils import FilterQueryError, error_response
...
DEFAULT_CONFIG = {
    ...
    "SNER_EXCLUSIONS": [["regex", r"^.*,proto=tcp,port=22,.*$"], ["network", "127.66.66.0/26"]],
    ...
    logging.config.dictConfig(
        {
            ...
            "formatters": {
                ...
                "formatter_werkzeug": {"format": "werkzeug %(message)s"},
            },
            "handlers": {
                "console_server": {"class": "logging.StreamHandler", "stream": "ext://sys.stdout", "formatter": "formatter_server"},
                "console_command": {"class": "logging.StreamHandler", "stream": "ext://sys.stdout", "formatter": "formatter_command"},
                "console_werkzeug": {"class": "logging.StreamHandler", "stream": "ext://sys.stdout", "formatter": "formatter_werkzeug"},
            },
            "loggers": {
                "sner.server": {"level": "INFO", "handlers": ["console_server"]},
                "sner_command": {"level": "INFO", "handlers": ["console_command"]},
                "werkzeug": {"handlers": ["console_werkzeug"]},
            },
        }
    )
