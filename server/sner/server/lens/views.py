...
def host_view_json_route(host_id):
    ...
    services = [
        ...
        for svc in sorted(host.services, key=lambda x: (x.proto, x.port, x.id))
    ]
    ...
    notes = [
        ...
        for note in sorted(host.notes, key=lambda x: (x.xtype, x.id))
    ]
    ...
    vulns = [
        ...
        for vuln in sorted(host.vulns, key=lambda x: ((x.service.port, x.service.proto) if x.service else tuple(), x.name, x.xtype, x.id))
    ]
...
def overview_json_route():
    ...
    return {
        ...
        "oldest_scanned": {
            "longest_ago": (datetime.utcnow() - oldest_scanned[0].import_time).days if oldest_scanned else None,
            "services": [
                {
                    "id": item.id,
                    "host_id": item.host_id,
                    "host_address": item.host.address,
                    "proto": item.proto,
                    "port": item.port,
