...
def initdata_sner():
    ...
    db.session.add(Queue(
        ...
        config=yaml_dump({'module': 'manymap', 'args': '-sV --version-intensity 4 -Pn -O', 'delay': 1}),
        group_size=50,
        priority=15,
    ))
    ...
    db.session.add(Queue(
        ...
        config=yaml_dump({'module': 'nmap', 'args': '-sU -sV -Pn -F -n --version-intensity 0 --max-retries 3 --open'}),
        group_size=50,
        priority=15,
    ))
