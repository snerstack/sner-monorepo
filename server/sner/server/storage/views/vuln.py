...
def vuln_multicopy_endpoints_json_route():
    ...
    data = {
        ...
        "data": sorted(hosts["data"] + services["data"], key=lambda item: (item["host_address"], item["service_port"] or 0)),
    }
...
def vuln_addedit_host_autocomplete_route():
    ...
    hosts = (
        Host.query.filter(or_(cast(Host.address, db.String).ilike(f"%{term}%"), Host.hostname.ilike(f"%{term}%")))
        .limit(current_app.config["SNER_AUTOCOMPLETE_LIMIT"])
        .all()
    )
    data = [{"value": host.id, "label": f"{host.address} (hostname: {host.hostname} id:{host.id})"} for host in hosts]
...
def vuln_addedit_service_autocomplete_route():
    ...
    services = Service.query.filter(
        Service.host_id == host_id, or_(Service.proto.ilike(f"%{service_term}%"), cast(Service.port, db.String).ilike(f"%{service_term}%"))
    ).all()
