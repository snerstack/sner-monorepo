...
from flask import Response, current_app, g, jsonify, redirect, session, url_for
...
def login_route(args):
    ...
    user = User.query.filter(User.active, User.username == args["username"]).one_or_none()

    if user:
        if "password" in args and args["password"]:
            if PWS.compare(PWS.hash(args["password"], PWS.get_salt(user.password)), user.password):
                if user.totp:
    ...
    current_app.logger.info("auth.login password failed, username=%s", args["username"])
...
def login_webauthn_route(args):
    ...
    try:
        assertion = cbor.decode(b64decode(args["assertion"]))
