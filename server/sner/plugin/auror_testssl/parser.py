...
"""
Parser for auror_testssl results.
"""
...
class ParserModule(ParserBase):  # pylint: disable=too-few-public-methods
    """auror_testssl parser class

    Attributes:
        ARCHIVE_PATHS (str): The regex pattern for archive paths.
        FINDINGS_IGNORE (list): A list of findings to ignore.
    """
    ...
    def parse_path(cls, path):
        ...
        Returns:
            ParsedItemsDb: The populated ParsedItemsDb instance.
        """
    ...
    def _parse_data(cls, data, pidb):
        ...
        Args:
            data (str): The raw string data to parse.
            pidb (ParsedItemsDb): The ParsedItemsDb instance to populate.
        ...
        pidb.upsert_note(host_address, f"auror.testssl_{tls_method}", service_proto, service_port, via_target, data=json.dumps(note_data))

        return pidb

    @classmethod
    def _process_testssl_result(cls, testssl_result):
        ...
        try:
            ...
            tls_method = "STARTTLS" if "--starttls" in processed_testssl_result["invocation"] else "SSL/TLS"
            ...
            for section in SECTIONS:
                for finding in scan_result.get(section, []):
                    ...
                    if finding["id"] == "cert" or finding["id"].startswith("intermediate_cert <#"):
                        processed_testssl_result[f"{finding['id']}_txt"] = cls._process_cert(finding["finding"])
    ...
    @classmethod
    def _process_cert(cls, raw_cert):
        ...
        """
        try:
            processed_cert = subprocess.run(["openssl", "x509", "-text", "-noout"], input=raw_cert, check=True, capture_output=True, text=True).stdout
