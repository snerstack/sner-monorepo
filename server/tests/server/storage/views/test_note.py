...
def test_note_list_json_route(cl_operator, note):
    ...
    response = cl_operator.post(url_for("storage.note_list_json_route", filter=f'Note.data=="{note.data}"'), {"draw": 1, "start": 0, "length": 1})
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
    assert response_data["data"][0]["data"] == note.data
...
def test_note_grouped_json_route(cl_operator, note):
    ...
    response = cl_operator.post(url_for("storage.note_grouped_json_route"), {"draw": 1, "start": 0, "length": 1, "search[value]": note.xtype})
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
    assert note.xtype in response_data["data"][0]["xtype"]

    response = cl_operator.post(
        url_for("storage.note_grouped_json_route", filter=f'Note.xtype=="{note.xtype}"'), {"draw": 1, "start": 0, "length": 1}
    )
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
    assert note.xtype in response_data["data"][0]["xtype"]
