...
def test_vuln_list_json_route(cl_operator, vuln):
    ...
    response = cl_operator.post(url_for("storage.vuln_list_json_route", filter=f'Vuln.name=="{vuln.name}"'), {"draw": 1, "start": 0, "length": 1})
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
    assert vuln.name in response_data["data"][0]["name"]
...
def test_vuln_grouped_json_route(cl_operator, vuln):
    ...
    response = cl_operator.post(url_for("storage.vuln_grouped_json_route"), {"draw": 1, "start": 0, "length": 1, "search[value]": vuln.name})
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
    assert vuln.name in response_data["data"][0]["name"]

    response = cl_operator.post(url_for("storage.vuln_grouped_json_route", filter=f'Vuln.name=="{vuln.name}"'), {"draw": 1, "start": 0, "length": 1})
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
    assert vuln.name in response_data["data"][0]["name"]
...
def test_vuln_grouped_json_route_tagaggregation(cl_operator, vuln_factory):
    ...
    response = cl_operator.post(url_for("storage.vuln_grouped_json_route"), {"draw": 1, "start": 0, "length": 10, "search[value]": vuln_name})
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
...
def test_vuln_multicopy_endpoints_json_route(cl_operator, vuln):
    ...
    response = cl_operator.post(url_for("storage.vuln_multicopy_endpoints_json_route"), {"draw": 1, "start": 0, "length": 1})
    assert response.status_code == HTTPStatus.OK
    response_data = json.loads(response.body.decode("utf-8"))
