...
def test_versioninfomanager_notequeryiterator(app, host, service_factory, note_factory):  # pylint: disable=unused-argument
    ...
    note_factory.create(host=host, service=service_factory.create(host=host, port=1), xtype="nmap.banner_dict", data='{"dummy": "1"}')

    note_factory.create(host=host, service=service_factory.create(host=host, port=2), xtype="nmap.banner_dict", data="invalid_dummy")
