...
from tests.selenium import dt_count_rows, dt_inrow_delete, dt_rendered, dt_wait_processing, frontend_url, wait_for_js, webdriver_waituntil
...
def test_vuln_list_route_service_endpoint_dropdown(frontend_server, sl_operator, vuln_factory, service):  # pylint: disable=unused-argument
    ...
    check_service_endpoint_dropdown(
        sl_operator, sl_operator.find_element(By.ID, "vuln_list_table"), f"{test_vuln.service.port}/{test_vuln.service.proto}"
    )

    sl_operator.find_element(By.XPATH, '//a[text()="svcTgt"]').click()
...
def test_vuln_list_route_moredata_dropdown(frontend_server, sl_operator, vuln):  # pylint: disable=unused-argument
    ...
    webdriver_waituntil(sl_operator, EC.visibility_of_element_located((By.XPATH, '//table[@id="vuln_list_table"]//h6[text()="More data"]')))
...
def test_vuln_list_route_viatarget_visibility_toggle(frontend_server, sl_operator, vuln):  # pylint: disable=unused-argument
    ...
    sl_operator.execute_script("sessionStorage.clear(); window.sessionStorage.setItem('dt_viatarget_visible', 'true');")
    sl_operator.refresh()
    wait_for_js(sl_operator)
    dt_rendered(sl_operator, "vuln_list_table", vuln.comment)
...
def test_vuln_edit_route_autocomplete(frontend_server, sl_operator, vuln, host_factory, service_factory):  # pylint: disable=unused-argument
    ...
    elem_xpath = (
        '//ul[@data-testid="service-autocomplete-list"]/li'
