...
def test_terminate_with_liveserver(tmpworkdir, live_server, apikey_agent, cleanup_markedprocess, longrun_target):  # noqa: E501  pylint: disable=unused-argument,redefined-outer-name
    """
    Agent external process handling test. Even thou the test uses nmap module, the point is to test sner.agent.modules.Base _terminate helper.
    """
    ...
    proc_agent = multiprocessing.Process(
        ...
        args=(
            [
                "--server",
                url_for("frontend.index_route", _external=True),
                "--apikey",
                apikey_agent,
                "--queue",
                db.session.get(Queue, longrun_target.queue_id).name,
                "--oneshot",
                "--debug",
            ],
        ),
...
def test_shutdown(tmpworkdir, httpserver):  # pylint: disable=unused-argument,redefined-outer-name
    ...
    proc_agent = multiprocessing.Process(target=agent_main, args=(["--server", sserver.url, "--apikey", "dummy", "--debug"],))
    proc_agent.start()
    sleep(1)
    assert proc_agent.is_alive()
